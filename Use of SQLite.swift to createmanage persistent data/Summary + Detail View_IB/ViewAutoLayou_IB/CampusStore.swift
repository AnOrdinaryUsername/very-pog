...
class CampusStore {
    ...
    func getStudents() -> [Student] {
        ...
        for r in rs {
            let stObj = try! Student(fn: r.get(database.fnCol), ln: r.get(database.lnCol), g: .male)
            stObj?.cwid = try! r.get(database.cwidCol)
            list.append(stObj!)
        }
        return list
    }
    
    func deleteStudent(sObj : Student) {
        print("Deleting student : \(sObj.cwid)")
        let tbl = database.studentTbl!
        let conn = database.conn!
        let filterTbl = tbl.filter(database.cwidCol == sObj.cwid)
